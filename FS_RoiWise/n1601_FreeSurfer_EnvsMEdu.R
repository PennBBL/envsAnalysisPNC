...
#volume <- read.csv("/data/joy/BBL/studies/pnc/n1601_dataFreeze/neuroimaging/t1struct/n1601_freesurferVol_20161220.csv")
...
vol <- read.csv("/data/joy/BBL/studies/pnc/n1601_dataFreeze/neuroimaging/t1struct/n1601_freesurferAsegVol_20161220.csv")

demo <- merge(demo, vol, by=c("bblid","scanid"))
...
FunctionAnalyze <- function(model, Path, cores) {
  ...
  demo <- merge(demo.ids, demo, by=c("bblid","scanid"))
...
path <- "/data/joy/BBL/studies/pnc/n1601_dataFreeze/neuroimaging/t1struct/n1601_jlfAntsCTIntersectionCT_20170331.csv"
outputjlfct <- FunctionAnalyze(model, path, cores)

path <- "/data/joy/BBL/studies/pnc/n1601_dataFreeze/neuroimaging/t1struct/n1601_freesurferAsegVol_20161220.csv"
vol <- read.csv(path)

vol <- vol[,-which(colMeans(vol) == 0)]
vol <- vol[,-which(names(vol) == "EstimatedTotalIntraCranialVol")]
path <- "~/n1601_fs_volaseg.csv"

write.csv(vol, path, row.names=F)

outputvol <- FunctionAnalyze(model, path, cores)

model <- "~ s(ageAtScan1, k=4) + sex + race2 + averageManualRating + EstimatedTotalIntraCranialVol"
outputvolicv <- FunctionAnalyze(model, path, cores)

model <- "~ s(ageAtScan1, k=4) + sex + race2 + averageManualRating + envSES + medu1"
outputvolboth <- FunctionAnalyze(model, path, cores)


model <- "~ s(ageAtScan1, k=4) + sex + race2 + averageManualRating + medu1 + envSES"

path <- "/data/joy/BBL/studies/pnc/n1601_dataFreeze/neuroimaging/t1struct/n1601_freesurferSurfaceArea_20161220.csv"
outputareaboth <- FunctionAnalyze(model, path, cores)

path <- "/data/joy/BBL/studies/pnc/n1601_dataFreeze/neuroimaging/t1struct/n1601_freesurferCt_20161220.csv"
ct <- read.csv(path)

ct$meanThickness <- rowMeans(ct[, c(37,72)])

demo <- merge(demo, ct, by=c("bblid","scanid"))

ct.model <- gam(meanThickness ~ s(ageAtScan1, k=4) + sex + race2 + averageManualRating + envSES, data=demo, method="REML")
summary(ct.model)
